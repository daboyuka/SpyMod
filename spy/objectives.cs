...
// ObjectiveMission::setObjectiveHeading() is in objectivesMods.cs
// Objective::displayBitmap is in objectivesMods.cs
...
// ObjectiveMission::initCheck is in objectivesMods.cs
...
// Mission::init is in objectiveMods.cs
...
function TowerSwitch::onCollision(%this, %object)
{
   ...
   %playerTeam = Client::getTeam(%playerClient);
...
function Flag::onDrop(%player, %type)
{
   ...
   Player::setItemCount(%player, Player::getMountedItem(%player, $FlagSlot), 0);
   %flag.carrier = -1;
   %player.carryFlag = "";
      ...
      else
      {
         ...
         if(%object.carryFlag != "")
         {
			   ...
			   if(%enemyTeam != -1)
            {
               ...
               Player::setItemCount(%object, GameBase::getDataName(%this), 0);
               %object.carryFlag = "";
   ...
   else
   {
      ...
      if(%object.carryFlag == "")
      {
			if(%object.outArea == "") {
        		...
        		Player::setItemCount(%object, GameBase::getDataName(%this), 1);
        		Player::mountItem(%object, GameBase::getDataName(%this), $FlagSlot, %flagTeam);
        		Item::hide(%this, true);
        		$flagAtHome[1] = false;
        		%this.atHome = false;
        		%this.carrier = %object;
...
function Flag::checkFlagsTime(%flag,%timeleft,%changeCount)
{
	if(%flag.changeTeamCount == %changeCount)
	{
		...
		if(%timeleft <= 0 && %flag.currentFlagStand == "") { 
			GameBase::startFadeOut(%flag);
      	Player::setItemCount(%flag.carrier, GameBase::getDataName(%flag), 0);
