...
function Elevator::onCollision(%this, %object)
{
	if(!Player::isDead(%object) && getObjectType(%object) == "Player")
		if (GameBase::getTeam(%this) == Client::getApparentTeam(GameBase::getOwnerClient(%object)) || %this.noTeam != "" || GameBase::getTeam(%this) == -1) {
...
function Door::onCollision(%this, %object)
{
	if(!Player::isDead(%object) && getObjectType(%object) == "Player") 
		if (GameBase::isActive(%this) && GameBase::isPowered(%this) && %this.faded == "")  
			if(GameBase::getTeam(%this) == Client::getApparentTeam(GameBase::getOwnerClient(%object)) || GameBase::getTeam(%this) == -1 || %this.noTeam != "" ) 
...
function Door::onDamage(%this,%type,%value,%pos,%vec,%mom,%object)
{
	...
	GameBase::setDamageLevel(%this,%damageLevel + %value * %TDS * DamageScale::getDamageScale(%this, %type));
...
{
	if(GameBase::getTeam(%this) == Client::getApparentTeam(Client::getOwnerClient(%object)) || GameBase::getTeam(%this) == -1 || %this.noTeam != "" ) {
		%this.status = "open";
		Door::onMove(%this);
	}
}
...
SoundData SoundHitTarget {
   wavFileName = "shieldhit.wav";
   profile = Profile3dFar;
};

SoundData SoundHitPlayerTarget {
   wavFileName = "player_death.wav";
   profile = Profile3dFar;
};

MoveableData Target6x6Octa {
	shapeFile = "elevator_6x6_octagon";
	className = "Target";
	destroyable = false;
	maxDamage = 1;
...
function Target::onAdd(%this) {
  ...
  if (%this.autoPopup) {
    Target::popupRepeat(%this, %this.popupInterval, %this.popupIntervalRand, %this.popupTime);
  }
}
...
function Target::onDestoryed(%this) {}

function Target::popup(%this, %time) {
  %this.popUpID++;
  %this.poppedUp = true;
  Moveable::moveForward(%this);
  schedule("if ("@%this@".poppedUp && "@%this@".popUpID == "@%this.popUpID@") { Moveable::moveBackward("@%this@"); "@%this@".poppedUp = \"\";}", %time);
}

function Target::unpopup(%this) {
  Moveable::moveBackward(%this);
  %this.poppedUp = "";
}

function Target::popupRepeat(%this, %interval, %intervalRand, %time) {
  Target::popup(%this, %time);
  schedule("Target::popupRepeat("@%this@","@%interval@","@%intervalRand@","@%time@");", %interval + %intervalRand*getRandom());
}

MoveableData MovingAntenna {
	shapeFile = "anten_small";
	className = "DumbMoveable";
	destroyable = false;
	maxDamage = 1;
	speed = 2;
	sfxStart = SoundElevatorStart;
