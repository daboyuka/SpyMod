...
function AIGraphEditor::linkMarkers() {
  if (!isObject(MESelectionSet)) return;
  for (%i = 0; %i < Group::objectCount(MESelectionSet); %i++) {
    %o = Group::getObject(MESelectionSet, %i);
    if (getObjectType(%o) != "Marker" || Object::getName(getGroup(%o)) != "AIGraph") return;
  }

  for (%i = 0; %i < Group::objectCount(MESelectionSet); %i++) {
    for (%j = %i+1; %j < Group::objectCount(MESelectionSet); %j++) {
      AIGraph::linkMarkers(Group::getObject(MESelectionSet, %i), Group::getObject(MESelectionSet, %j));
    }
  }
...
function AIGraph::getClosestMarkerToAI(%aiName, %markerGroup) {
  %minDist = 100000000;
  %minDistMarker = -1;
...
function AIGraph::printAdjacencyList(%markerGroup) {
  for (%i = 0; %i < Group::objectCount(%markerGroup); %i++) {
    %m = Group::getObject(%markerGroup, %i);
