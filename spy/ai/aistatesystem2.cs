...
$SuperAI::PATH_TYPE_CIRCUIT = 0;
$SuperAI::PATH_TYPE_ONE_WAY = 1;
$SuperAI::PATH_TYPE_TWO_WAY = 2;

////////////////////
// Spawn functions
////////////////////

sethelp(SuperAI::Spawn, "name armor position rotation customScript ?marker ?team",
       "Spawns an AI called <name> with <armor> at <position> with <rotation>. This AI will use the SuperAI script " @
       "<customScript>, which will be passed <marker> if specified. The AI will be added to <team> if specified.");
function SuperAI::Spawn(%name, %armor, %pos, %rot, %customScript, %marker, %team) {
  if (%name == "" || %armor == "" || %pos == "" || %rot == "" || %customScript == "") {
    help(SuperAI::Spawn);
    return;
  }
...
function SuperAI::changeState(%aiName, %newState, %numArgs, %arg0, %arg1, %arg2, %arg3, %arg4, %arg5, %arg6, %arg7) {
  %oldState = SuperAI::getState(%aiName);
...
// If the periodic function returns a value that does not evaluate to false,
// %interval will be ignored and the return value will be used for the schedule
// time instead. If the periodic function always returns its scheduling value,
...
function SuperAI::registerEventForWaypoint(%aiName, %directive, %position, %isLastWaypoint) {
  ...
  %cmd = "function " @ %funcName @ "(%aiName) {" @
           "schedule(\"if (SuperAI::getState(\\\"\" @ %aiName @ \"\\\") == " @ SuperAI::getState(%aiName) @ ") {" @
             "SuperAI::fireEvent(\\\"\"@%aiName@\"\\\", waypointReached, 3, " @ %directive @ ", \\\"" @ %position @ "\\\", " @ tern(%isLastWaypoint, true, false) @ ");" @
             "deleteFunctions(" @ %funcName @ ");" @
             "}\",0);" @
         "}";
...
function SuperAI::movePath(%aiName, %waypointSet, %pathType) {
  SuperAI::cancelMove(%aiName);
  %ret = "";

  if (%pathType == "") %pathType = 0;
  AI::setVar(%aiName, "pathType", %pathType);
...
function AI::onTargetLOSAcquired(%aiName, %target) { SuperAI::fireEvent(%aiName, "targetLOSAcquired", 2, %target, false); }
function AI::onTargetLOSRegained(%aiName, %target) { SuperAI::fireEvent(%aiName, "targetLOSAcquired", 2, %target, true); }
